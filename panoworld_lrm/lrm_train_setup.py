...
try:
    import wandb
except ImportError:  # wandb is optional for the open-source training entry.
    wandb = None
import shutil
import copy
...
def init_wandb_and_backup(config):
    ...
    if not config.training.get("use_wandb", False):
        return None

    if wandb is None:
        raise ImportError("wandb is not installed, but training.use_wandb is true.")

    api_key_path = config.training.get("api_key_path", "")
    if api_key_path:
